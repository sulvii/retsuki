...
export default class KissCommand extends Command {
	override async run(ctx: CommandContext<typeof options>) {
		...
		const nekoResponse = await ctx.client.neko.fetch("kiss", 1);
		const result = nekoResponse.results[0];

		if (!result) {
			return ctx.editOrReply({
				content: `Could not fetch kiss GIF :<`,
			});
		}
		...
		const embed = new Embed()
			...
			.setTitle(
				`${ctx.author.globalName || ctx.author.username} kisses ${user.globalName || user.username}! How cute (. ❛ ᴗ ❛.)`,
			)
			.setImage(result.url)
