...
import { generateLocation, LocationType, Regions, type Location } from "../../../db/data/locations";
    ...
    async run(ctx: CommandContext<typeof options>) {
        ...
        if (existing) {
            const embed = new Embed()
                .setColor(0xFFB7C5)
                .setDescription("❌ You already have a kingdom! Use `/kingdom stats` to view it~")
        ...
        const displayName = ctx.guildId
            ? (await (await ctx.client.guilds.fetch(ctx.guildId)).members.fetch(ctx.author.id)).displayName
            : ctx.author.username;
